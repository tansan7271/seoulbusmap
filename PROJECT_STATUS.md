...
- **4.4. 보고서 '백엔드' 파트 작성 및 스케줄러 로직 개선**
  - **목표:** 보고서의 '데이터베이스 설계' 및 '백엔드' 파트 내용을 구체화하고, 데이터 수집 주기의 효율성을 높이기 위해 스케줄러 로직을 수정.
  - **보고서 작성 지원:**
    - '데이터베이스 설계' 및 '백엔드' 챕터의 도입부 문단을 작성하여 내용의 흐름을 자연스럽게 연결.
    - `HangJeongDong`, `BusStop`, `BusData` 각 모델의 목적을 설명하는 핵심 설명 문구 작성.
    - `apps.py`의 스케줄러가 '지속 가능한 운영 환경'이라는 프로젝트 핵심 목표를 어떻게 달성하는지에 대한 상세한 설명과 기술적 문제 해결 과정을 포함하여 '백엔드' 파트의 내용을 대폭 보강.
  - **스케줄러 로직 개선 (`main/apps.py`):**
    - **문제점 인식:** 버스 정류장 정보와 행정동 정보가 모두 '월별' 주기로 묶여 있어, 데이터 성격에 맞는 유연한 수집 주기 설정이 필요했음.
    - **해결 과정:**
      - `run_monthly_scheduler`를 `run_hangjeongdong_scheduler`로 변경하고 행정동 데이터 수집만 담당하도록 수정.
      - `run_busstop_scheduler` 함수를 신설하여 버스 정류장 데이터는 '주간' 단위로 수집하도록 분리.
      - `ready()` 메서드에서 각 스케줄러(일간, 주간, 월간)가 1초의 간격을 두고 순차적으로 시작되도록 코드를 리팩토링하여, 기존의 복잡한 체이닝 로직을 제거하고 가독성과 안정성을 향상시킴.
  - **결과:** 보고서의 핵심 파트 내용이 구체화되었으며, 데이터 자동 수집 스케줄러가 각 데이터의 특성에 맞게 더 효율적이고 관리하기 쉬운 구조로 개선됨.
